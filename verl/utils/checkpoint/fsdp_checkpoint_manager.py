...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, path=None, del_local_after_load=False, *args, **kwargs):
        ...
        with FSDP.state_dict_type(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            ...
            print("e2 len(model_state_dict.keys()): ", len(model_state_dict.keys()))

            new_model_state_dict = {}
            for k, v in model_state_dict.items():
                if not any([_ in k for _ in ["model.language_model.", "model.visual."]]):
                    new_k = k.replace("model.", "model.language_model.").replace("visual.", "model.visual.")
                    new_model_state_dict[new_k] = v
            model_state_dict = new_model_state_dict if len(new_model_state_dict) > 1 else model_state_dict # lm_head.weight

            print("e3 len(model_state_dict.keys()): ", len(model_state_dict.keys()))

            self.model.load_state_dict(model_state_dict)
            if self.optimizer is not None:

                print("optimizer_state_dict.keys(): ", optimizer_state_dict.keys())
