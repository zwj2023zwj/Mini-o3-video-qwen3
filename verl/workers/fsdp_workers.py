...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(self,
        ...
        with init_context(), warnings.catch_warnings():
            ...
            if actor_model_config.architectures[0] == "TTVFMForConditionalGeneration":
                from verl.models.ttvfm_hf import TTVFMForConditionalGeneration
                actor_module_class = TTVFMForConditionalGeneration
    ...
    def _build_rollout(self):
        ...
        if self.config.rollout.name == 'hf':
        ...
        elif self.config.rollout.name == 'vllm_multi_turn_tool_call':
            if self.config.rollout.use_fire_sampling:
                # FIXME: use_fire_sampling does not support vLLMRollout_MultiTurn_ToolCall
                from verl.workers.rollout.vllm_rollout import FIREvLLMRollout as vLLMRollout
                from verl.workers.rollout.vllm_rollout import vllm_mode
            else:
                from verl.workers.rollout.vllm_rollout import vLLMRollout, vLLMRollout_MultiTurn_ToolCall, vllm_mode
            from verl.workers.sharding_manager import FSDPVLLMShardingManager
            log_gpu_memory_usage('Before building vllm_multi_turn rollout', logger=None)
            local_path = copy_to_local(self.config.model.path)
            if vllm_mode == 'customized':
                # FIXME: 'customized' does not support vLLMRollout_MultiTurn_ToolCall
                rollout = vLLMRollout(actor_module=self.actor_module_fsdp,
                                      config=self.config.rollout,
                                      tokenizer=self.tokenizer,
                                      model_hf_config=self.actor_model_config)
                ...
                extra_info = {
                    "acc_reward_weight": self.config.data.get("acc_reward_weight", 1.0),
                    "format_reward_weight": self.config.data.get("format_reward_weight", 1.0),
                    "use_tool_reward_weight": self.config.data.get("use_tool_reward_weight", 0.0),
                    "tool_call_penalty": self.config.data.get("tool_call_penalty", 0.1),
                    "extract_answer_tags": self.config.data.get("extract_answer_tags", "split"),
                    "general_qa_reward_fn": self.config.data.get("general_qa_reward_fn", "v1"),
                    "gpt_extract_answer": self.config.data.get("gpt_extract_answer", False),
                    "model_system_prompt": system_prompt,
                    "max_total_response_length": self.config.rollout.max_total_response_length,
                    "overlong_buffer_len": self.config.reward_model.get("overlong_buffer_len", 0),
                }
                tokenizer = self.tokenizer
                reward_fn = reward_manager_cls(tokenizer=tokenizer, num_examine=0, compute_score=compute_score, mode="train", extra_info=extra_info, gpt_threads=self.config.data.get("gpt_threads", 100))

                # Note that we always use function-based RM for validation
                val_reward_fn = reward_manager_cls(tokenizer=tokenizer, num_examine=1, compute_score=compute_score, mode="val", extra_info=extra_info, gpt_threads=self.config.data.get("gpt_threads", 100))

                gpt_reward_fn = None
                val_gpt_reward_fn = None

                use_hybrid_reward_manager = self.config.reward_model.get("use_hybrid_reward_manager", False)
                if use_hybrid_reward_manager:
                    from verl.workers.reward_manager import NaiveMultiThreadsToolRewardManager
                    import copy
                    gpt_extra_info = copy.deepcopy(extra_info)
                    gpt_extra_info['general_qa_reward_fn'] = self.config.data.get("gpt_general_qa_reward_fn", "general_qa_tool")

                    # import pdb; pdb.set_trace()

                    gpt_reward_fn = NaiveMultiThreadsToolRewardManager(tokenizer=tokenizer, num_examine=0, compute_score=compute_score, mode="train", extra_info=gpt_extra_info, gpt_threads=self.config.data.get("gpt_threads", 100))
                    val_gpt_reward_fn = NaiveMultiThreadsToolRewardManager(tokenizer=tokenizer, num_examine=1, compute_score=compute_score, mode="val", extra_info=gpt_extra_info, gpt_threads=self.config.data.get("gpt_threads", 100))

                from verl.workers.rollout.vllm_rollout import vLLMAsyncRollout
                vllm_rollout_cls = vLLMRollout_MultiTurn_ToolCall if self.config.rollout.mode == "sync" else vLLMAsyncRollout
                rollout = vllm_rollout_cls(model_path=local_path,
                                      config=self.config.rollout,
                                      tokenizer=self.tokenizer,
                                      processor=self.processor, # Add processor
                                      model_hf_config=self.actor_model_config,
                                      device_mesh=rollout_device_mesh,
                                      reward_fn=reward_fn,
                                      gpt_reward_fn=gpt_reward_fn,
                                      val_reward_fn=val_reward_fn,
                                      val_gpt_reward_fn=val_gpt_reward_fn,
            ...
            rollout_sharding_manager = FSDPVLLMShardingManager(module=self.actor_module_fsdp,
                                                               ...
                                                               device_mesh=rollout_device_mesh)
            log_gpu_memory_usage('After building sharding manager', logger=None)
        elif self.config.rollout.name == 'vllm_multi_turn_resize_image':
            if self.config.rollout.use_fire_sampling:
                # FIXME: use_fire_sampling does not support vLLMRollout_MultiTurn_ResizeImage
                from verl.workers.rollout.vllm_rollout import FIREvLLMRollout as vLLMRollout
                from verl.workers.rollout.vllm_rollout import vllm_mode
            else:
                from verl.workers.rollout.vllm_rollout import vLLMRollout, vLLMRollout_MultiTurn_ResizeImage, vllm_mode
            from verl.workers.sharding_manager import FSDPVLLMShardingManager
            log_gpu_memory_usage('Before building vllm_multi_turn rollout', logger=None)
            local_path = copy_to_local(self.config.model.path)
            if vllm_mode == 'customized':
                # FIXME: 'customized' does not support vLLMRollout_MultiTurn_ResizeImage
                rollout = vLLMRollout(actor_module=self.actor_module_fsdp,
                                      config=self.config.rollout,
                                      tokenizer=self.tokenizer,
                                      model_hf_config=self.actor_model_config)
            elif vllm_mode == 'spmd':
                rollout = vLLMRollout_MultiTurn_ResizeImage(model_path=local_path,
                                      config=self.config.rollout,
                                      tokenizer=self.tokenizer,
                                      processor=self.processor, # Add processor
                                      model_hf_config=self.actor_model_config,
        ...
        if self.config.rollout.mode == "sync":
            ...
            load_dtensor_weights(
                params, rollout_sharding_manager.inference_engine.llm_engine.model_executor.driver_worker.worker.model_runner.model)

        if self.device_mesh is not None:
            self.torch_random_states = torch.cuda.get_rng_state()
            torch.cuda.set_rng_state(rollout_sharding_manager.gen_random_states)
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        print("----fsdp_worker_rollout generate_sequence time: ", time.time())
        ...
        print("----fsdp_worker generate_sequence time: ", time.time())
        return output
    
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_log_prob_and_update_actor(self, data: DataProto):
        '''
        self.rollout_sharding_manager.inference_engine.sleep(level=1)
        # self.rollout_sharding_manager.module.train()
        ...
        if self.device_mesh is not None:
            self.rollout_sharding_manager.gen_random_states = torch.cuda.get_rng_state()
            torch.cuda.set_rng_state(self.rollout_sharding_manager.torch_random_states)
        '''
        ...
        params = self.actor_module_fsdp.state_dict()
        log_gpu_memory_usage('After state_dict() in sharding manager memory', logger=logger)
        self.rollout_sharding_manager.inference_engine.wake_up()
        from verl.third_party.vllm import load_dtensor_weights
        load_dtensor_weights(
            params, self.rollout_sharding_manager.inference_engine.llm_engine.model_executor.driver_worker.worker.model_runner.model)
        del params
...
class AsyncActorRolloutRefWorker(ActorRolloutRefWorker):
    def _build_rollout(self):
        rollout, rollout_sharding_manager = super()._build_rollout()
